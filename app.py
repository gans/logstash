...
class App:
    ...
    def __init__(self, rds_client, s3_client, debug=False):
        ...
        logger.info("start retrivieng rds log")
        self.last_written = self.get_last_s3_written()
        logger.info("last s3 written is {}".format(self.last_written))
        logs_desc = self.get_logs_metadata()
        if logs_desc:
            logs_desc = self.sort_last_first(logs_desc)
            logger.info("found logs to download, initializing")
            self.save_last_written(logs_desc)
            self.download_log(logs_desc)
        else:
            logger.info("not found logs to download, ending")
            
        logger.info("ending retrieving rds logs")


    def get_last_s3_written(self):
        try:
            response = self.s3_client.head_bucket(Bucket=conf.aws_bucket_instance)
        ...
        try:
            response = self.s3_client.get_object(Bucket=conf.aws_bucket_instance, Key=S3_CONFIG_FILE)
            j = response["Body"].read(response["ContentLength"])
            j = json.loads(j)
            logger.info("config file was sucessfull written on s3")
            return j["LastWritten"] + 1
        except botocore.exceptions.ClientError as e:
            error_code = int(e.response["ResponseMetadata"]["HTTPStatusCode"])
            if error_code == 404:
                logger.info("config file was not found, using default for first time")
                return 0
            else:
    ...
    def get_logs_metadata(self):
        ...
        while more_logs:
            response = self.rds_client.describe_db_log_files(
                DBInstanceIdentifier=conf.aws_rds_instance,
    ...
    def save_last_written(self, logs_desc):
        ...
        last = logs_desc[-1]
        try:
            response = self.s3_client.put_object(Bucket=conf.aws_bucket_instance,
                                                Key=S3_CONFIG_FILE,
                                                Body=json.dumps(last))
        except botocore.exceptions.ClientError as e:
            logger.error("could not write config file on s3: {}".format(
                    e.response["Error"]["Message"]))
    ...
    def download_log(self, logs):
        ...
        for log in logs:
            ...
            logger.info("downloading {}".format(id_file_log))
            ...
            log_file = dwl(conf.aws_rds_instance, log["LogFileName"], "0")
            ...
            logger.info("upload finished {}".format(id_file_log))
    ...
    def create_s3_mpu(self, file_name):
        return self.s3_client.create_multipart_upload(
                    Bucket=conf.aws_bucket_instance,
